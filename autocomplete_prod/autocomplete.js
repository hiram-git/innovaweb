let arr;
function autocomplete(inp, CodAlmacen) {
  
  if((sessionStorage.getItem('product_av')=='') || (sessionStorage.getItem('product_av')==null)){
    load_data();
  }else{
    //alert(sessionStorage.getItem('clientes_av'));
    arr=sessionStorage.getItem('product_av').split('|');
  }
      ...
      for (i = 0; i < arr.length; i++) {
        ...
        if (arr[i].substr(0, val.length).toUpperCase() == val.toUpperCase()) {
          ...
          /*execute a function when someone clicks on the item value (DIV element):*/
              ...
              //alert(inp.value);
              buscar_prod(inp.value, CodAlmacen.value);
...
function load_data(){
    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            arr=editor.split('|');
            sessionStorage.setItem('product_av', editor);
    ...
    xhttp.open("GET", "autocomplete_prod/load_data.php", true);
    xhttp.send();
    //return tString;
}
/*
