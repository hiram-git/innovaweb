...
$txt_buscar = str_replace("", "*", $txt_buscar);
$cad_sql="";
$vendedor = isset($_SESSION['codvendedor_opt']) 
    ? $_SESSION['codvendedor_opt'] 
    : (isset($_SESSION['codvendedor']) ? $_SESSION['codvendedor'] : '');

$sql_inv_vendedor = "SELECT CODPRO FROM INVENTARIOVEN WHERE CODVEN = '{$vendedor}';";
$sentencia_inv_vendedor = $base_de_datos->prepare($sql_inv_vendedor);
$sentencia_inv_vendedor->execute();
$inv_vendedor = $sentencia_inv_vendedor->fetchAll(PDO::FETCH_COLUMN);
$app_inv_vend = "";
$app_inv_vend_doble = "";
if($inv_vendedor){
    $formattedString = "'" . implode("','", $inv_vendedor) . "'";
    $app_inv_vend = ' AND INVENTARIO.CODPRO IN ('.$formattedString.')';
    $formattedStringDouble = "''" . implode("'',''", $inv_vendedor) . "''";
    $app_inv_vend_doble = ' AND INVENTARIO.CODPRO IN ('.$formattedStringDouble.')';
}
...
, MAX(CASE 
        WHEN INVENTARIOVEN.EXISTENCIA IS NOT NULL 
            AND INVENTARIOVEN.EXISTENCIA <> 0 
        THEN INVENTARIOVEN.EXISTENCIA 
        ELSE INVENTARIO.CANVEN 
    END) AS CANVEN
...
LEFT JOIN INVENTARIOVEN on
INVENTARIO.CODPRO = INVENTARIOVEN.CODPRO
WHERE 
...
INVENTARIO.DESCRIP1 ASC');   ";
    ...
    INVENTARIO.CODPRO = INVENTARIOVEN.CODPRO
    WHERE 
    INVENTARIO.ACTIVO=0 $app_inv_vend
    GROUP BY INVENTARIO.CODPRO
    , INVENTARIO.CODIGO
    , INVENTARIO.DESCRIP1
    , INVENTARIO.DESCRIP2
    ...
    <div id='bton' class='tabcontent'>
        ...
        "; 
            ...
            while ($data2 = $sentencia4->fetchObject()){
                ...
                $disponible=$data2->EXISTENCIA-$data2->RESERVADA-$data2->CANVEN;
				...
				$SQL_CHECK_LOTES = "    SELECT COUNT(*) as count
							FROM INFORMATION_SCHEMA.COLUMNS 
							WHERE TABLE_NAME = 'INVENTARIOLOTES' 
							AND COLUMN_NAME LIKE 'EXISTENCIA".$txt_CodAlmacen."'";
				$result_check_lotes = $base_de_datos->query($SQL_CHECK_LOTES);
				$CHECK_LOTES = $result_check_lotes->fetchColumn();
				if($CHECK_LOTES == 1){
                ...
                if($_SESSION['tipo_tarea']=='presupuesto'){
                    ?>
                        <button type='button' class='boton_prod' style='background-color:<?php echo $bgcolor;?>;' onClick="cargar_data('<?php echo $data2->CODPRO;?>', '<?php echo $nombre_formateado;?>', '<?php echo round($data2->Precio_fijo, 2);?>', '<?php echo $data2->IMPPOR;?>', '<?php echo $data2->COSTOACT;?>', '<?php echo $data2->COSTOPRO;?>', '<?php echo $data2->GRUPOINV;?>', '<?php echo $data2->CODDEP;?>', '<?php echo $data2->LINEAINV;?>', '<?php echo $data2->Precio_fijo;?>', '<?php echo $disponible;?>', '<?php echo $_SESSION['tipo_tarea'];?>', '<?php echo $data2->EXENTO;?>','','<?php echo $data2->TIPINV;?>','<?php echo $data2->PROCOMPUESTO;?>')">
                            ...
                            <i class="fa fa-check" aria-hidden="true" style="color:#1ab744;"></i> 
                            Existencias: <span><?php echo round($data2->EXISTENCIA-$data2->CANVEN);?></span><br />
                            <i class="fa fa-check" aria-hidden="true" style="color:#1ab744;"></i> 
                            Reservado: <span><?php echo round($data2->RESERVADA);?></span>
                            <i class="fa fa-check" aria-hidden="true" style="color:#1ab744;"></i>  
                            Disponible: <span><?php echo $disponible;?></span></b>
                                <br />
                                <div class='titulo_prod' style="display: flex;align-items: center;justify-content: center;border:0px solid #ccc;"><b>
                                    <div style="background-color:#dcddd4;width:100px;height:60px;border-radius:10px;display: inline-grid;padding:5px;">Precio: <?php echo "<span style='color:#fa6630;'>".number_format(round($precio_, 2), 2)."</span>";?></div>
                                </b></div>
                            </div>   
                    ...
                    //if($total_reg20==''){$_SESSION['actfacexi'] 
                        $style_habilitar = "";
                        if(isset($_SESSION["actfacexi"]) AND $_SESSION["actfacexi"] == "0"){
                            
                            $style_habilitar="pointer-events: none;color:#F1F1F1;";
                        }
                        if($data2->TIPINV == 1 OR $disponible>0){
                            
                            $style_habilitar="";
                            
                        }
                        if(($data2->TIPINV == 1 OR $disponible>0) OR (isset($_SESSION["ventamenos"]) AND $_SESSION["ventamenos"] == "1") OR (isset($_SESSION["actfacexi"]) AND $_SESSION["actfacexi"] == "1")  OR $data2->PROCOMPUESTO == "1"){
                            ?>
                            <button type='button' class='boton_prod' style='background-color:<?php echo $bgcolor;?>;<?= $style_habilitar?>' onClick="cargar_data('<?php echo $data2->CODPRO;?>', '<?php echo $nombre_formateado;?>', '<?php echo number_format(round($data2->Precio_fijo, 2), 2, '.', '');?>', '<?php echo $data2->IMPPOR;?>', '<?php echo $data2->COSTOACT;?>', '<?php echo $data2->COSTOPRO;?>', '<?php echo $data2->GRUPOINV;?>', '<?php echo $data2->CODDEP;?>', '<?php echo $data2->LINEAINV;?>', '<?php echo $data2->Precio_fijo;?>', '<?php echo $disponible;?>', '<?php echo $_SESSION['tipo_tarea'];?>', '<?php echo $data2->EXENTO;?>','','<?php echo $data2->TIPINV;?>','<?php echo $data2->PROCOMPUESTO;?>')">
                            <div style='font-size:16px;text-align:left;color:black;'><b><?php echo $data2->CODPRO;?> <i class="fa fa-long-arrow-right" aria-hidden="true"></i> <?php echo $data2->DESCRIP1;?></b></div>
                                <div style='font-size:16px;text-align:center;' class='titulo_prod'>
                                <b style='font-size:16px;'>
                                <i class="fa fa-check" aria-hidden="true" style="color:#1ab744;"></i>
                                ...
                                Disponible: <span><?php echo $disponible;?></span></b>
                                <br />
                                <div class='titulo_prod' style="display: flex;align-items: center;justify-content: center;"><b>
                                    <div style="background-color:#dcddd4;width:100px;height:60px;border-radius:10px;display: inline-grid;padding:5px;">Precio: <?php echo "<span style='color:#fa6630;'>".number_format(round($precio_, 2), 2)."</span>";?></div> 
                                    </b>
                                    <?php if( $data2->CANTIDAD_EMP > 0 ){ ?>
                                        <div style="background-color:#dcddd4;width:100px;height:60px;border-radius:10px;display: inline-grid;padding:5px;" class="gray-button" id="btn_empaque" data-id="<?php echo $data2->CODPRO;?>" data-almacen="<?php echo $_GET["CodAlmacen"]?>" ><b>Empaque</b></div> 
                                    <?php } elseif( $data2->CANTIDAD_EMP <= 0  && $data2->PROCOMPUESTO == "1" ) { ?>
                                        <div style="background-color:#dcddd4;width:100px;height:60px;border-radius:10px;display: inline-grid;padding:5px;" class="gray-button" id="btn_comp" data-id="<?php echo $data2->CODPRO;?>" data-almacen="<?php echo $_GET["CodAlmacen"]?>" ><b>Comp.</b></div> 
                                    <?php } ?>
                            ...
                            ?>
                            <button type='button' class='boton_prod' style='background-color:<?php echo $bgcolor;?>;<?= $style_habilitar?>' onClick="swal('Alerta!', 'Producto sin existencia ', {buttons:false,});">                  
                            <div style='font-size:16px;text-align:left;color:black;'><b><?php echo $data2->CODPRO;?> <i class="fa fa-long-arrow-right" aria-hidden="true"></i> <?php echo $data2->DESCRIP1;?></b></div>
                                <div style='font-size:16px;text-align:center;' class='titulo_prod'>
                                <b style='font-size:16px;'>
                                <i class="fa fa-check" aria-hidden="true" style="color:#1ab744;"></i>
                                ...
                                <i class="fa fa-check" aria-hidden="true" style="color:#1ab744;"></i>
                                Disponible: <span><?php echo $disponible;?></span></b>
                                <br /><div class='titulo_prod' style="display: flex;align-items: center;justify-content: center;"><b>
                                <div style="background-color:#dcddd4;width:100px;height:60px;border-radius:10px;display: inline-grid;padding:5px;">Precio: <?php echo "<span style='color:#fa6630;'>".number_format(round($precio_, 2), 2)."</span>";?></div> 
                                </b></div>
                                </div>
