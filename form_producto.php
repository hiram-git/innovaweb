...
<body>
  ...
  <main>
      ...
      <div class="container"  style='border:0px solid #000;'>
        <div style="padding:10px 0px 10px 0px;border:0px solid #ccc;width:100%;">
          <div class='titulo'>
                ...
                <h5 style='color:#fff;'>ASIGNAR PRODUCTOS<br /><?php echo strtoupper($_SESSION['tipo_tarea']);?></h5><hr/>
    ...
    $sentencia4->execute();
    $data2 = $sentencia4->fetchObject();
    $pormaxdespar=$data2->PORMAXDESPAR;
    
    if($_SESSION['mtodesctoglo'] > $pormaxdespar)
    {
      $pormaxdespar = $_SESSION['mtodesctoglo'];
    }
      ...
      <table border='0' style='color:#fff;' cellpadding='0'>
          ...
          <td colspan="3">
              <!-- <div class="row height d-flex justify-content-center align-items-center"> -->
                  <!-- <div class="col-md-6"> -->
                      ...
                      <input type='hidden' id='actfacexi' value="<?php echo $_SESSION["actfacexi"];?>">
        ...
        <tr>
          ...
          while ($data23 = $sentencia43->fetchObject()){
            ...
            $precio_menor_baseempresa=$data23->PVPMENOR;
			    ...
			    $sql33="SELECT CODUSER, CLAVE, VALVENDEDOR, VALDEPOSITO, ACTPRECIO, VALPRECIO, CREACLIENTE, ACTCLIENTE, VALCLIENTE,
          ACTVENDEDOR, VALVENDEDOR,ACTDEPOSITO, VALDEPOSITO, ACTDESCTOPAR, ACTDESCTOGLOBAL, CAMBIARPRECIO, VENTAMENOS, ACTFACEXI, MODIFPRECIOS
          FROM BASEUSUARIOS WHERE UPPER(CODUSER)='".$_SESSION['coduser']."'";
          $sentencia43 = $base_de_datos->prepare($sql33, [
            PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL,
            ]);
          ...
          if($_SESSION['usuario_precio']=='libre'){
            ...
            if(($cambiar_precio==1) OR ($_SESSION['usuario_precio']=='no_definido')){ //validando si permite editar precio
        ...
        <tr>
            <td colspan="3">Precio: <br />
              ...
              if($_SESSION['usuario_precio']=='libre'){
                $k=1;
                echo "<div class='padre' style='border:0px solid #ccc;display: flex;margin: 5px;justify-content: center;flex-wrap: wrap;'>";
                foreach($arr_precio AS $k_pre => $pre){
                  //echo "$k_pre => $pre";
                  if($k_pre==0){
                    ?>
